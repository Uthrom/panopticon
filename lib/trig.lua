...
function trig.PointWithinShape(shape, tx, ty)
	...
	elseif #shape == 2 then 
		return trig.PointWithinLine(shape, tx, ty)
	else 
		return trig.CrossingsMultiplyTest(shape, tx, ty)
	end
end
 
function trig.BoundingBox(box, tx, ty)
...
function trig.colinear(line, x, y, e)
	e = e or 0.1
...
function trig.PointWithinLine(line, tx, ty, e)
	e = e or 0.66
	if trig.BoundingBox(line, tx, ty) then
		return trig.colinear(line, tx, ty, e)
	else
		return false
	end
end
 ...
 * _point_, returns 1 if inside, 0 if outside.
 --]]
function trig.CrossingsMultiplyTest(pgon, tx, ty)
...
function trig.GetIntersect( points )
	local g1 = points[1].x
	local h1 = points[1].y
	...
	if trig.checkIntersect({x=g1, y=h1}, {x=g2, y=h2}, {x=i1, y=j1}, {x=i2, y=j2}) then
		local a = h2-h1
		local b = (g2-g1)
